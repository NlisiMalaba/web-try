...
$(document).ready(function() {
    ...
    function sendMessage(customMessage) {
        ...
        // Process batch if it's full or if it's been long enough since last batch
        if (messageBatch.length >= BATCH_SIZE || (Date.now() - lastMessageTime > BATCH_DELAY)) {
            processMessageBatch();
        }
    }
    ...
    function processMessageBatch() {
        ...
        const batch = messageBatch.slice(0, BATCH_SIZE);
        messageBatch = messageBatch.slice(BATCH_SIZE);
        ...
        // Add typing indicator only for the first message in batch
        if (batch.length > 0) {
            showTypingIndicator();
        }
        ...
        const promises = batch.map(msg => {
            return sendSingleMessage(msg.message);
        });
    ...
    function addMessage(sender, content, isHistory = false) {
        ...
        if (!isHistory) {
            ...
            // Only animate if not adjacent to last message
            if ($messages.length === 0 || $messages.length > 0 && lastMessage.data('sender') !== sender) {
